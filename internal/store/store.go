...
func (s *Store) AddTodo(openID, title string) (int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	res, err := s.db.Exec("INSERT INTO todos(open_id,title,status,created_at) VALUES(?,?,?,?)",
...
func (s *Store) SetTodoStatus(id int64, openID, status string) (bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	res, err := s.db.Exec("UPDATE todos SET status=? WHERE id=? AND open_id=?", status, id, openID)
	if err != nil {
		return false, err
	}
